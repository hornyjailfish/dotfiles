...
local custom_filter = require("s.window-picker.custom-window-filter")
...
return {
	...
	keys = {
		{
			...
			function()
				local picked_window_id = require('window-picker').pick_window({
					...
					--    return {1000, 1001}
					-- end
					filter_rules = custom_filter.make_rules(true),
					picker_config = {
...

		{
			...
			function()
				local picked_window_id = require('window-picker').pick_window({
					filter_rules = custom_filter.make_rules(),
					picker_config = {
						statusline_winbar_picker = {
							selection_display = selection_display
						},
	...
	config = function()
		...
		require("window-picker").setup({
			autoselect_one = true,
			include_unfocusabel_windows = false,
			...
			filter_func = custom_filter.filter_func
		})
	end,
}
